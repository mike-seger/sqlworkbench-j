...
public class TableListParserTest
{
  ...
  @Test
  public void testGetTablesLateral()
  {
    TableListParser parser = new TableListParser(ParserType.Postgres);
    String sql = "select * from table_one t1 join lateral unnest(t1.x) as foo(id)";
    List<Alias> l = parser.getTables(sql, true);
    assertEquals(2, l.size());
    assertEquals("t1", l.get(0).getAlias());
    assertEquals("table_one", l.get(0).getObjectName());
    assertEquals("foo", l.get(1).getAlias());

    sql =
      "select *\n" +
      "from status st\n" +
      "  left join lateral jsonb_each(st.data) as d(code, flag) on true";
    l = parser.getTables(sql, true);
    assertEquals(2, l.size());
    assertEquals("st", l.get(0).getAlias());
    assertEquals("d", l.get(1).getAlias());
  }
  ...
  public void testGetTables()
  {
    ...
    assertEquals(2, l.size());
    assertEquals("t1", l.get(0).getAlias());
    assertEquals("t2", l.get(1).getAlias());
  }
