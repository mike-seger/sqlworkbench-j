  ...
  extends WbTestCase
{
  ...
  @Test
  public void testGetMergeTable()
  {
    String sql = "merge into foobar using (select * from x) t on (t.id = foobar.id) when matched then update set bla = 42;";
    String table = SqlUtil.getMergeTable(sql, null);
    assertEquals("foobar", table);

    sql = "merge into public.foobar as x using (select * from x) t on (t.id = foobar.id) when matched then update set bla = 42;";
    table = SqlUtil.getMergeTable(sql, null);
    assertEquals("public.foobar", table);
  }
