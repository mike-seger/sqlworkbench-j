  ...
  extends WbTestCase
{
    ...
    throws Exception
  {
    ...
    for (Object o : data)
    {
      ...
      names.add(((TableIdentifier)o).getTableName());
    }
  ...
  @Test
  public void testColumnsInWhere()
  {
    String sql = "select * from one as o join two on one.id1 = two.id2 where ";
    int pos = sql.indexOf("where") + "where".length() + 1;
    StatementContext context = new StatementContext(con, sql, pos);
    BaseAnalyzer analyzer = context.getAnalyzer();
    assertTrue(analyzer instanceof SelectAnalyzer);
...
//    System.out.println("columns: " + columns);
    ...
    assertEquals("o.LASTNAME", columns.get(2).toString());
    assertEquals("two.ID2", columns.get(3).toString());
    assertEquals("two.SOME_DATA", columns.get(4).toString());
  }
