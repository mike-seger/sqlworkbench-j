  ...
  extends WbTestCase
{
  ...
  public void testUsingOperator() throws SQLException
  {
    ...
    List<JoinCondition> conditions = creator.getJoinConditions();
    assertEquals(1, conditions.size());
    JoinCondition cond = conditions.get(0);
    String condition = cond.getJoinCondition(true, QuoteHandler.STANDARD_HANDLER);
    assertEquals("USING (per_id)", condition);

    cond.setPreferUsingOperator(false);
    condition = cond.getJoinCondition(true, QuoteHandler.STANDARD_HANDLER);
    ...
    throws Exception
  {
    ...
    String condition = creator.getJoinConditions().get(0).getJoinCondition();
    ...
    condition = creator.getJoinConditions().get(0).getJoinCondition(false, QuoteHandler.STANDARD_HANDLER);
    ...
    assertEquals("address", join.getObjectName());
    assertEquals("ad", join.getAlias());
    assertEquals("adt.type_id = ad.adr_type_id", creator.getJoinConditions().get(0).getJoinCondition(false, QuoteHandler.STANDARD_HANDLER).trim());
    ...
    String cond2 = creator.getJoinConditions().get(0).getJoinCondition(false, QuoteHandler.STANDARD_HANDLER).trim();
