  ...
  extends WbTestCase
{
    ...
    throws Exception
  {
    ...
    assertEquals("GENERATED BY DEFAULT AS IDENTITY (START WITH 12 INCREMENT BY 2 MINVALUE 12 CYCLE)", columns.get(1).getGenerationExpression());

    assertFalse(columns.get(2).isAutoincrement());
    assertFalse(columns.get(2).isIdentityColumn());
    assertTrue(columns.get(2).isGenerated());
    assertEquals("GENERATED ALWAYS AS (ID * 2)", columns.get(2).getGenerationExpression());
  }
}
