...
public class ConstantColumnValues
{
  ...
  /**
   * The variable indicating the line number of the input file.
   *
   * It is merely stored here, but not processed (or replaced) when using {@link #setParameter(PreparedStatement, int, int)}
   * To check if a "constant" value is a line number, use {@link #isLineNumber(int)}
   */
    ...
    throws SQLException, ConverterException
  {
    ...
    Set<String> varNames = CollectionUtil.caseInsensitiveSet(VAR_NAME_CURRENT_FILE_DIR, VAR_NAME_CURRENT_FILE_NAME, VAR_NAME_CURRENT_FILE_PATH, VAR_NAME_CURRENT_LINE);
    ...
    for (String entry : entries)
    {
      ...
      String value = StringUtil.trimToNull(entry.substring(pos + 1));
      ...
      if (col != null)
      {
        ...
        if (value == null)
        {
          LogMgr.logWarning(new CallerInfo(){}, "Empty value for column '" + col + "' assumed as NULL");
        }
        else
        {
          if (isLineNumber(value) || value.startsWith("${") || value.startsWith("$@{"))
          {
            // DBMS Function call or line number variable, we can't convert that to a real value
            data = value;
          }
          else
          {
            // Remove single quotes around string literals
            if (SqlUtil.isCharacterType(col.getDataType()) && value.charAt(0) == '\'' && value.charAt(value.length() - 1) == '\'')
            {
  ...
  /**
   * Checks if the given "constant" value is in fact a variable for the line number.
   * @param index  - the index to the constant value definition.
   */
  public boolean isLineNumber(int index)
  {
    Object value = getValue(index);
    if (value instanceof String)
    {
      return isLineNumber((String)value);
    }
    return false;
  }
  ...
  public void setParameter(PreparedStatement pstmt, int indexInStatement, int columnIndex)
    throws SQLException
  {
    ...
    {
      pstmt.setObject(indexInStatement, value);
    }
  }
