...
public class TableStatements
{
    ...
    throws SQLException
  {
    ...
    try
    {
      ...
      LogMgr.logDebug(new CallerInfo(){}, "Executing statement: " + sql);
      stmt.execute(sql);
    ...
    catch (SQLException e)
    {
      ...
      if (ignoreErrors)
      {
        LogMgr.logWarning(new CallerInfo(){}, errMsg + ": " + e.getMessage());
      }
      else
      {
        LogMgr.logError(new CallerInfo(){}, errMsg + sql, e);
        throw new TableStatementError(e, tbl);
      }
    }
    catch (Throwable th)
    {
      LogMgr.logError(new CallerInfo(){}, "Error running statement: " + sql, th);
      con.rollback(sp);
    }
    finally
    {
