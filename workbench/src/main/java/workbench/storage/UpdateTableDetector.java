...
public class UpdateTableDetector
{
  ...
  public void checkUpdateTable(TableIdentifier table, ResultInfo resultInfo)
  {
    ...
    if (checkPkOnly)
    {
      LogMgr.logDebug(ci, "Only checking the PK definition for " + tbl.getTableExpression());
      checkPkOnlyForUpdateTable(tbl, resultInfo);
      if (updateTable != null)
      {
        LogMgr.logDebug(ci, "Using update table: " + updateTable.getTableExpression());
        return;
      }
    ...
    try
    {
      ...
      {
        LogMgr.logWarning(ci, "No columns from the table " + this.updateTable.getTableExpression() + " could be found in the current result set!");
      }
    ...
    catch (Exception e)
    {
      ...
      LogMgr.logError(ci, "Could not read table definition", e);
    }

    LogMgr.logDebug(ci, "Using update table: " + updateTable);
  ...
  private int findColumn(String columnName, ResultInfo info)
  {
    ...
    {
      LogMgr.logWarning(new CallerInfo(){}, "Column " + columnName + " is not unique. Ignoring column!");
      return -1;
    }
    return index;
  }
  ...
  private void checkPkOnlyForUpdateTable(TableIdentifier tbl, ResultInfo resultInfo)
  {
    ...
    LogMgr.logInfo(new CallerInfo(){}, "Retrieving primary key for table " + tbl.getTableExpression() + " took: " + duration + "ms");
  ...
  private void checkUniqueIndexesFor(TableIdentifier tableToUse, ResultInfo result)
  {
    ...
    LogMgr.logInfo(new CallerInfo(){}, "No PK found for table " + tableToUse.getTableName()+ " Trying to find an unique index.");
    ...
    long duration = System.currentTimeMillis() - start;
    LogMgr.logInfo(new CallerInfo(){}, "Retrieving unique indexes for table " + tableToUse.getTableExpression() + " took: " + duration + "ms");
    ...
    if (idx == null)
    {
      LogMgr.logDebug(new CallerInfo(){}, "No unique index found for table: " + tableToUse.getTableExpression());
      return;
    }

    LogMgr.logInfo(new CallerInfo(){}, "Using unique index " + idx.getObjectName() + " as a surrogate PK for table: " + tableToUse.getTableExpression());
  ...
  public void syncResultColumn(int index, ColumnIdentifier column, ResultInfo info)
  {
    ...
    if (column.getGeneratedColumnType() != GeneratedColumnType.none)
    {
      canUpdate = false;
      LogMgr.logDebug(new CallerInfo(){}, "Column " + column.getColumnName() + " can not be updated because it is a computed/generated column");
    }

    if (info.getColumn(index).isReadonly())
    {
      LogMgr.logDebug(new CallerInfo(){}, "Column " + column.getColumnName() + " was marked as read-only by the driver!");
    }

    ColumnIdentifier resultCol = info.getColumn(index);
    resultCol.setGeneratedExpression(column.getGenerationExpression(), column.getGeneratedColumnType());
    resultCol.setIsPkColumn(column.isPkColumn());
    resultCol.setIsNullable(column.isNullable());
    resultCol.setUpdateable(canUpdate);
  }
