             ...
             DbData, DbUpdater, Interruptable, JobErrorHandler, PropertyChangeListener, StatusBar
{
  ...
  public DwPanel(DwStatusBar statusBar)
  {
    ...
    dataTable.setShowRemarksInHeader(GuiSettings.showRemarksInTableHeader());
    initColors();
  }
  ...
  private TableCheck checkUpdateTable()
  {
    ...
    try
    {
      ...
      if (!updateTableFound)
      {
        ...
        if (dataTable.getDataStore().getResultInfo().columnTablesAvailable())
        {
        ...
        else
        {
          ...
          List<String> tableColumns = detector.getColumnsFromTable(tbl, sql, dbConnection);
          if (CollectionUtil.isEmpty(tableColumns))
          {
            checkResult = TableCheck.noTableColumns;
          }
        }

        if (checkResult == TableCheck.tableOk)
        {
  ...
  public boolean startEdit(boolean restoreSelection)
  {
    ...
    if (!updateable && (checkResult == TableCheck.noTable || checkResult == TableCheck.noTableColumns))
    {
      ...
      if (checkResult == TableCheck.noTableColumns)
      {
        WbSwingUtilities.showErrorMessageKey(w, "MsgNoTableColumns");
      }
      else
      {
