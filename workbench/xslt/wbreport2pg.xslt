...
<!--
  Parameters can be overriden by using -xsltParameter
  For example: -xsltParameter="useJdbcTypes=true" sets param useJdbcTypes to true.
-->
  ...
  <xsl:template match="/">
    <xsl:message>Supported parameters:
...
* includeIndexes       - generate DDL for index definitions (current value: <xsl:value-of select="$includeIndexes"/>)
    ...
    <xsl:for-each select="column-def">
      ...
      <xsl:variable name="datatype">
        ...
        <xsl:if test="$useJdbcTypes = 'false'">
          <xsl:choose>
            <xsl:when test="$dbms-typename = 'clob' or $dbms-typename = 'longvarchar' or $dbms-typename = 'lvarchar' or $dbms-typename = 'long varchar'">
            ...
            <xsl:when test="java-sql-type-name = 'VARCHAR' or java-sql-type-name = 'NVARCHAR'">
              <xsl:choose>
      ...
      <xsl:variable name="defaultvalue">
        <xsl:if test="default-value = 'autoincrement'">
          <xsl:value-of select="' GENERATED BY DEFAULT AS IDENTITY'"/>
        </xsl:if>
        <xsl:if test="string-length(default-value) &gt; 0 and default-value != 'autoincrement'">
  ...
  <xsl:template name="write-data-type">
    ...
    <xsl:if test="$auto-inc = 'true'">
      <xsl:text> GENERATED BY DEFAULT AS IDENTITY</xsl:text>
    </xsl:if>
  </xsl:template>
